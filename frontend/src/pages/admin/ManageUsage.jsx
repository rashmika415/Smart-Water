...
import { Search, Eye, ChevronLeft, ChevronRight, Droplets, Leaf, Home, Activity, RefreshCw, Download } from "lucide-react";

function MetricCard({ label, value, helper, icon: Icon, tone = "bg-sky-50 text-sky-700" }) {
  return (
...
function HouseholdDetailsModal({ open, data, loading, error, onClose }) {
  ...
  useEffect(() => {
    ...
    setFromDate("");
    setToDate("");
  }, [open, data?.household?._id]);

  const activityOptions = useMemo(() => {
    const set = new Set((data?.records || []).map((r) => String(r?.activityType || "")).filter(Boolean));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return (data?.records || []).filter((record) => {
      const matchesActivity = activityFilter === "all" || String(record?.activityType || "") === activityFilter;
      const matchesSource = sourceFilter === "all" || String(record?.source || "manual") === sourceFilter;
      const occurred = record?.occurredAt ? new Date(record.occurredAt) : null;

      const matchesFrom = !start || (occurred && occurred >= start);
      const matchesTo = !end || (occurred && occurred <= new Date(`${toDate}T23:59:59`));

      return matchesActivity && matchesSource && matchesFrom && matchesTo;
    });
  }, [data?.records, activityFilter, sourceFilter, fromDate, toDate]);

  const exportDetailsCsv = useCallback(() => {
    if (!filteredRecords.length) return;

    const headers = ["Date", "Activity", "Liters", "CarbonKg", "Source"];
    const rows = filteredRecords.map((record) => [
      record?.occurredAt ? new Date(record.occurredAt).toISOString() : "",
      record?.activityType || "",
      Math.round(Number(record?.liters || 0)),
    ...
    a.href = url;
    a.download = `household-usage-details-${new Date().toISOString().slice(0, 10)}.csv`;
      ...
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" size="sm" variant="ghost" onClick={exportDetailsCsv} disabled={!filteredRecords.length} className="gap-1.5">
                      ...
                      {filteredRecords.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-slate-500">
                            No records match the selected filters. Try wider dates or clear activity/source filters.
                          </td>
                        </tr>
                      ) : (
...
export function ManageUsage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, days, sort, limit]);

  useEffect(() => {
    setJumpPage(String(rowsData.page || 1));
  }, [rowsData.page]);
  ...
  const refreshAll = useCallback(async () => {
    await Promise.all([loadOverview(), loadAnomalies(), loadHouseholds()]);
  }, [loadOverview, loadAnomalies, loadHouseholds]);

  const rangeLabel = useMemo(() => {
    if (!rowsData.total) return "Showing 0 results";
    const start = (rowsData.page - 1) * limit + 1;
    const end = Math.min(rowsData.page * limit, rowsData.total);
    return `Showing ${start}-${end} of ${rowsData.total} households`;
  }, [rowsData.total, rowsData.page, limit]);

  const applyJumpPage = useCallback(() => {
    const parsed = Number(jumpPage || 1);
    const target = Math.max(1, Math.min(Number(rowsData.totalPages || 1), parsed));
    setPage(target);
  }, [jumpPage, rowsData.totalPages]);

  const getSuspiciousSeverity = useCallback((item) => {
    const threshold = Number(anomalies?.thresholds?.spikeLitersThreshold || 0);
    const peak = Number(item?.maxSingleRecordLiters || 0);
    if (!threshold || peak >= threshold * 2) return { label: "High", tone: "bg-rose-100 text-rose-700" };
    if (peak >= threshold * 1.25) return { label: "Medium", tone: "bg-amber-100 text-amber-700" };
    return { label: "Low", tone: "bg-emerald-100 text-emerald-700" };
  }, [anomalies?.thresholds?.spikeLitersThreshold]);
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <Card className="p-5">
          ...
          <div className="mt-1 text-xs text-slate-500">Liters consumed per day in selected period (hover bars for exact value)</div>
                ...
                <div key={item.date} className="text-center">
                  <div className="mx-auto flex h-24 w-6 items-end rounded-full bg-slate-100 p-1">
                    <div
                      ...
                      style={{ height: `${pct}%` }}
                      title={`${item?.date || "-"}: ${Number(item?.totalLiters || 0).toLocaleString()} L`}
                    />
                  </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500">
            <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-sky-500" /> Daily liters bars</span>
            <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-slate-300" /> Highest day scales chart</span>
          </div>
        </Card>
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-2">
            {activitySummary.length === 0 ? (
              <div className="rounded-xl bg-slate-50 px-3 py-4 text-sm text-slate-500">No data for this period. Try switching to last 90 days or refreshing overview.</div>
            ) : (
          ...
          <div>
            ...
            <div className="space-y-2">
              {loadingAnomalies ? (
                ...
                <div className="rounded-xl bg-emerald-50 px-3 py-4 text-sm text-emerald-700">No suspicious household pattern detected. This usually means usage is within expected baseline.</div>
              ) : (
                (anomalies?.suspiciousHouseholds || []).slice(0, 5).map((item) => (
                  <div key={item.householdId} className="rounded-xl bg-amber-50 px-3 py-3 ring-1 ring-amber-100">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${getSuspiciousSeverity(item).tone}`}>
                        {getSuspiciousSeverity(item).label}
                      </span>
                    </div>
                    <div className="mt-1 text-xs text-slate-600">
          ...
          <div>
            ...
            <div className="space-y-2">
              {loadingAnomalies ? (
                ...
                <div className="rounded-xl bg-emerald-50 px-3 py-4 text-sm text-emerald-700">No extreme spike records in this period. Try a wider date range for deeper audit.</div>
              ) : (
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">Window: {days} days</span>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">Sort: {sort}</span>
          {debouncedSearch ? (
            <span className="rounded-full bg-brand-100 px-2.5 py-1 text-xs font-semibold text-brand-800">Search: {debouncedSearch}</span>
          ) : null}
        </div>
      </Card>
              ...
              {loadingRows ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-slate-500">
                    No household usage data found. Try clearing filters or expanding the date window.
                  </td>
                </tr>
              ) : (
                rowsData.rows.map((row, idx) => (
                  <tr key={row.householdId} className={idx % 2 === 0 ? "hover:bg-slate-50/80" : "bg-slate-50/30 hover:bg-slate-50/80"}>
                    <td className="px-4 py-3 font-medium text-slate-900">{row.householdName || "-"}</td>
                    <td className="px-4 py-3 text-slate-600">{row.city || "-"}</td>
                    <td className="px-4 py-3 text-right">{Number(row.residents || 0)}</td>
                    <td className="px-4 py-3 text-right">{Number(row.totalLiters || 0).toLocaleString()}</td>
                    <td className="px-4 py-3 text-right">{Number(row.totalCarbonKg || 0).toFixed(3)}</td>
                    <td className="px-4 py-3 text-right">{Number(row.usageCount || 0).toLocaleString()}</td>
                    <td className="px-4 py-3 text-slate-600">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2 py-1.5">
            ...
            <input
              ...
              className="w-12 bg-transparent text-center text-xs font-semibold text-slate-700 outline-none"
            />
            <span className="text-xs text-slate-500">/ {rowsData.totalPages || 1}</span>
            <button
              type="button"
