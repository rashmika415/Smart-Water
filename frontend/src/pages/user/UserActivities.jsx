...
import { Calendar, MapPin, Clock, CheckCircle2, Timer, AlertCircle, Info, Shovel } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-5xl pb-20">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-1 rounded-2xl bg-slate-100 p-1 ring-1 ring-slate-200">
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2">
        {loading ? (
          ...
          <div className="col-span-full flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 py-20">
             ...
             <p className="mt-1 text-xs text-slate-500">There are no maintenance activities matching your filter.</p>
          </div>
        ) : (
          filtered.map((a) => {
      ...
      <div className="mt-12 rounded-3xl bg-brand-600 p-8 text-white shadow-xl shadow-brand-200 relative overflow-hidden">
         <div className="relative z-10">
            ...
            <p className="mt-2 text-sm text-brand-100 max-w-md">
              If you notice a leak or system issue that isn't listed here, please contact our support team immediately.
            </p>
            <Button variant="white" className="mt-6 text-brand-700 font-black">
               Contact Support
            </Button>
         </div>
         <Droplet className="absolute right-[-20px] bottom-[-20px] h-48 w-48 text-white/10 -rotate-12" />
      </div>
    </div>
  );
}
